...
import { getItemFromLocalStorage, setItemToLocalStorage } from "@helpers/flow-manager/local-storage";
...
export const resetFlow = ({ flow, pathToAgreementId }: ResetFlowParams) => (props: object):void => {
...
export const withResetFlowHandlers = ({ flow, pathToAgreementId }: ResetFlowParams): WithResetFlowHOC =>
    withHandlers({
        resetAgreementFlow: props => () =>
            resetFlow({ flow, pathToAgreementId })(props),
    });